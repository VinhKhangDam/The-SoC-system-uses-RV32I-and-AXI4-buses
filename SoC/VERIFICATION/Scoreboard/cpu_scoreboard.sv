...
class cpu_scoreboard extends uvm_scoreboard;
    ...
    function bit periph_default_expected(bit [31:0] addr, output bit [31:0] exp);
        case (addr)
            32'h2000_0000: begin exp = 32'h0000_0000; return 1'b1; end
            32'h2000_0004: begin exp = 32'h0000_0000; return 1'b1; end
            32'h3000_000C: begin exp = 32'd115200;    return 1'b1; end
            32'h4000_0004: begin exp = 32'h0000_0008; return 1'b1; end
            32'h4000_000C: begin exp = 32'd10;        return 1'b1; end
            default: begin
                exp = 32'h0000_0000;
                return 1'b0;
            end
    ...
    function void report_axi_read_compare(
        string pname,
        bit [31:0] addr,
        bit [31:0] expected,
        bit [31:0] actual
        ...
        if (expected !== actual) begin
            ...
            axi_read_fails++;
            `uvm_error("SCB_FAIL",
                $sformatf("[%s] READ FAIL Addr=%h Expected=%h Got=%h",
                    pname, addr, expected, actual))
        end else begin
            axi_read_passes++;
            `uvm_info("SCB_PASS",
                $sformatf("[%s] READ PASS Addr=%h Expected=%h Got=%h",
                    pname, addr, expected, actual),
                UVM_LOW)
        end
    endfunction
    ...
    function void load_expected(string path);
        ...
        if (fd == 0) begin
            `uvm_warning("SCB",
                $sformatf("Cannot open expected file: %s - scoreboard will skip register comparison", path))
            expected_loaded = 0;
            return;
        end
        ...
        if (reg_idx == 32) begin
            expected_loaded = 1;
            `uvm_info("SCB",
                $sformatf("Loaded expected registers from: %s", path),
                UVM_LOW)
        end else begin
            expected_loaded = 0;
            `uvm_warning("SCB",
    ...
    virtual function void write_axi(axi_transaction tr);
        ...
        if (tr.is_write) begin
            if (tr.bresp != 2'b00) begin
                axi_write_errors++;
                `uvm_error("SCB_AXI",
                    $sformatf("AXI WRITE ERROR: Addr=%h Data=%h BRESP=%b",
                        addr, data, tr.bresp))
            end
        end else begin
            if (tr.rresp != 2'b00) begin
                axi_read_errors++;
                `uvm_error("SCB_AXI",
                    $sformatf("AXI READ ERROR: Addr=%h Data=%h RRESP=%b",
                        addr, data, tr.rresp))
            end
        end
        ...
        if (addr >= 32'h1000_0000 && addr < 32'h2000_0000) begin
            if (tr.is_write) begin
                ...
                dram_writes++;
                `uvm_info("SCB_MEM",
                    $sformatf("[DRAM] WRITE Addr=%h Data=%h", addr, data),
                    UVM_HIGH)
            end else begin
                dram_reads++;
            ...
            if (tr.is_write) begin
                if (shadows_peripheral_write(addr))
                    periph_shadow[addr] = data;

                `uvm_info("SCB_PERIPH",
                    $sformatf("[%s] WRITE Addr=%h Data=%h", pname, addr, data),
                    UVM_LOW)
            end else if (is_volatile_read(addr)) begin
                axi_read_skips++;
                `uvm_info("SCB_PERIPH",
                    $sformatf("[%s] READ Addr=%h Data=%h (volatile, compare skipped)",
                        pname, addr, data),
                    UVM_LOW)
                ...
                axi_read_skips++;
                `uvm_info("SCB_PERIPH",
                    $sformatf("[%s] READ Addr=%h Data=%h (no expected value)",
                        pname, addr, data),
                    UVM_LOW)
            end
        end
    ...
    virtual function void check_phase(uvm_phase phase);
        ...
        for (int i = 0; i < 32; i++) begin
            ...
            `uvm_info("SCB",
                $sformatf("| x%02d |  %08h  |  %08h  | %-5s | %-20s |",
                    i, exp_val, act_val, match_str, status),
                UVM_LOW)
        end
        ...
        `uvm_info("SCB",
            ...
            UVM_LOW)
        `uvm_info("SCB",
            $sformatf(" AXI Data Errors  : %-3d   DRAM W/R : %0d/%0d   TIMER:%0d UART:%0d SPI:%0d",
                axi_data_errors, dram_writes, dram_reads,
                timer_accesses, uart_accesses, spi_accesses),
            UVM_LOW)
        `uvm_info("SCB",
            $sformatf(" AXI Read Checks  : PASS:%0d FAIL:%0d SKIP:%0d",
                axi_read_passes, axi_read_fails, axi_read_skips),
            UVM_LOW)
        `uvm_info("SCB", "==============================================================", UVM_LOW)
        `uvm_info("SCB", "", UVM_LOW)
        ...
        if (fail_count > 0)
            ...
            `uvm_error("SCB",
                $sformatf("SIMULATION FAILED - AXI errors: %0d write, %0d read, %0d data",
                    axi_write_errors, axi_read_errors, axi_data_errors))
