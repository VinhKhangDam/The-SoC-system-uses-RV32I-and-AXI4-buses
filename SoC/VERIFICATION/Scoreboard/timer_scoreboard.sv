...
class timer_scoreboard extends uvm_scoreboard;
   ...
   function void tick_model();
      if (exp_control[0]) begin
         if (exp_period != 32'h0 && exp_count >= exp_period)
           exp_count = 32'h0;
         else
   ...
   function bit count_matches(bit [31:0] actual);
      ...
      if (exp_period != 32'h0 && actual <= exp_period)
        return 1'b1;

      if (exp_period == 32'h0) begin
         for (int i = 0; i < 8; i++) begin
            if (actual == (exp_count + i))
              return 1'b1;
            if (exp_count >= i && actual == (exp_count - i))
              return 1'b1;
         end
         return 1'b0;
      end
      ...
      for (int i = 0; i < 4; i++) begin
         ...
         if (exp_period != 32'h0 && probe >= exp_period)
           probe = 32'h0;
         else
           probe = probe + 1'b1;
      end
      ...
      case (tr.addr[3:0])
         ...
         4'h8: begin
            ...
            if (count_matches(tr.rdata)) begin
               ...
               count_check_count++;
               `uvm_info("TIMER_SCB",
                  $sformatf("COUNT PASS got=%h model=%h control=%h period=%h",
                     tr.rdata, exp_count, exp_control, exp_period),
                  UVM_LOW)
            end else begin
               fail_count++;
               `uvm_error("TIMER_SCB",
                  $sformatf("COUNT FAIL got=%h model=%h control=%h period=%h",
                     tr.rdata, exp_count, exp_control, exp_period))
            end
            return;
         end
   ...
   function void check_phase(uvm_phase phase);
      if (fail_count == 0)
        `uvm_info("TIMER_SCB",
                  $sformatf("TIMER PASS pass=%0d fail=%0d writes=%0d reads=%0d ignored_writes=%0d count_reads=%0d count_checks=%0d",
                            pass_count, fail_count, write_count, read_count, ignored_write_count,
                            count_read_count, count_check_count),
                  UVM_LOW)
      else
        `uvm_error("TIMER_SCB",
                   $sformatf("TIMER FAIL pass=%0d fail=%0d writes=%0d reads=%0d ignored_writes=%0d count_reads=%0d count_checks=%0d",
                             pass_count, fail_count, write_count, read_count, ignored_write_count,
                             count_read_count, count_check_count))
