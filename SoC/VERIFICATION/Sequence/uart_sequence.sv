  ...
  task body();
    ...
    repeat (200) begin
      ...
      assert (tr.randomize() with {
        ...
        if (addr == 32'h3000_000C) data inside {32'd4, 32'd8, 32'd16, 32'd32};

        if (op == UART_RX_BYTE) baud_cycles == cur_baud;

        wstrb == 4'hf;
      });
      finish_item(tr);

      if (tr.op == UART_AXI_WRITE && tr.addr == 32'h3000_000C) cur_baud = tr.data;
    end
  endtask  // body
