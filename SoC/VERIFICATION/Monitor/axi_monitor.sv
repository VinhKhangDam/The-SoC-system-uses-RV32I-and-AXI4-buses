  ...
  task collect_write_transactions();
    forever begin
      ...
      if (vif.awvalid && vif.awready) begin
        ...
        // Capture W channel
        while (!(vif.wvalid && vif.wready)) @(posedge vif_cr.clk);
        tr.data  = vif.wdata;
        tr.wstrb = vif.wstrb;
        ...
        `uvm_info("MON_WR", $sformatf("Detected WRITE : Addr=%h Data=%h Strb=%b Prot=%b BResp=%b",
                                      tr.addr, tr.data, tr.wstrb, tr.awprot, tr.bresp), UVM_LOW)
  ...
  task collect_read_transactions();
    forever begin
      ...
      if (vif.arvalid && vif.arready) begin
        ...
        `uvm_info("MON_RD", $sformatf("Detected READ  : Addr=%h Data=%h Prot=%b RResp=%b", tr.addr,
                                      tr.data, tr.arprot, tr.rresp), UVM_LOW)
        item_collected_port.write(tr);
      end
    end
  endtask
