...
class cpu_monitor extends uvm_monitor;
  ...
  virtual function void build_phase(uvm_phase phase);
    ...
    if (!uvm_config_db#(virtual soc_inf)::get(this, "", "vif_soc", axi_vif))
  ...
  task monitor_pipeline();
    forever begin
      ...
      if (cpu_vif.mon_cb.PcF !== last_pc_f && !cpu_vif.mon_cb.StallF) begin
        ...
        `uvm_info("IF", $sformatf("PC=%h  INSTR=%h  [%s]%s", cpu_vif.mon_cb.PcF,
                                  cpu_vif.mon_cb.InstrF, decode_instr(cpu_vif.mon_cb.InstrF),
                                  cpu_vif.mon_cb.mem_stall_i ? " <AXI_STALL>" : ""), UVM_MEDIUM)
      end

      // ---- DECODE: log Rs1/Rs2/Rd ----
      if (cpu_vif.mon_cb.InstrD !== '0) begin
        `uvm_info("ID", $sformatf("PC=%h  rs1=x%0d rs2=x%0d rd=x%0d  [%s]", cpu_vif.mon_cb.PcD,
                                  cpu_vif.mon_cb.Rs1D, cpu_vif.mon_cb.Rs2D, cpu_vif.mon_cb.RdD,
                                  decode_instr(cpu_vif.mon_cb.InstrD)), UVM_HIGH)
      end

      // ---- EXECUTE: log ALU result + forwarding ----
      if (cpu_vif.mon_cb.RdE !== '0) begin
        `uvm_info("EX", $sformatf("rd=x%0d  ALU=%h  Fwd=[%b,%b]%s", cpu_vif.mon_cb.RdE,
                                  cpu_vif.mon_cb.ALUResultE, cpu_vif.mon_cb.ForwardA,
                                  cpu_vif.mon_cb.ForwardB, cpu_vif.mon_cb.PCSrc ? $sformatf
                                  ("  *** BRANCH/JUMP TAKEN → flush ***") : ""), UVM_HIGH)
      end

      // ---- MEMORY: log store ----
      if (cpu_vif.mon_cb.MemWriteM) begin
        `uvm_info("MEM", $sformatf("STORE Addr=%h Data=%h", cpu_vif.mon_cb.ALUResultM,
                                   cpu_vif.mon_cb.WriteDataM), UVM_MEDIUM)
      end
      ...
      if (cpu_vif.mon_cb.RegWriteW && cpu_vif.mon_cb.RdW !== '0
        ...
        `uvm_info("WB", $sformatf("x%02d <= %h  (PC~%h)", tr.rd, tr.result, tr.pc), UVM_LOW)

        wb_port.write(tr);
      end

      // ---- HAZARD: log stalls / flushes ----
      if (cpu_vif.mon_cb.StallF) `uvm_info("HAZ", "STALL (fetch frozen)", UVM_HIGH)
      if (cpu_vif.mon_cb.FlushE) `uvm_info("HAZ", "FLUSH (ID/EX bubble inserted)", UVM_HIGH)
    end
  endtask
  ...
  task monitor_axi_writes();
    forever begin
      ...
      if (axi_vif.mon_cb.awvalid && axi_vif.mon_cb.awready) begin
        ...
        while (!(axi_vif.mon_cb.wvalid && axi_vif.mon_cb.wready)) @(axi_vif.mon_cb);
        tr.data  = axi_vif.mon_cb.wdata;
        tr.wstrb = axi_vif.mon_cb.wstrb;

        while (!(axi_vif.mon_cb.bvalid && axi_vif.mon_cb.bready)) @(axi_vif.mon_cb);
        // FIX: was inside wrong block scope due to misaligned 'end'
        tr.bresp = axi_vif.mon_cb.bresp;

        `uvm_info("AXI_WR", $sformatf("CPU -> %s  Addr=%h Data=%h Strb=%b BRESP=%b",
                                      get_slaves_name(tr.addr), tr.addr, tr.data, tr.wstrb,
                                      tr.bresp), UVM_LOW)
  ...
  task monitor_axi_reads();
    forever begin
      ...
      if (axi_vif.mon_cb.arvalid && axi_vif.mon_cb.arready) begin
        ...
        while (!(axi_vif.mon_cb.rvalid && axi_vif.mon_cb.rready)) @(axi_vif.mon_cb);
        tr.data  = axi_vif.mon_cb.rdata;
        tr.rresp = axi_vif.mon_cb.rresp;

        `uvm_info("AXI_RD", $sformatf("CPU <- %s  Addr=%h Data=%h RRESP=%b", get_slaves_name(
                                      tr.addr), tr.addr, tr.data, tr.rresp), UVM_LOW)
