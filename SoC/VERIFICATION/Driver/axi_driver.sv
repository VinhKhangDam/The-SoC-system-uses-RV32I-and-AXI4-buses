class axi_driver extends uvm_driver #(axi_transaction);
  ...
  task reset_bus();
    ...
    vif.drv_cb.bready  <= 0;
    vif.drv_cb.rready  <= 0;
    if (vif_cr.rstn !== 1'b1) @(posedge vif_cr.rstn);
    @(vif.drv_cb);
  endtask
  ...
  task drive_write(axi_transaction tr);
    ...
    `uvm_info("DRV", $sformatf("Write Done: Addr=%h Data=%h Prot=%b", tr.addr, tr.data, tr.awprot),
              UVM_MEDIUM)
  endtask
  ...
  task drive_read(axi_transaction tr);
    ...
    `uvm_info("DRV", $sformatf("Read Done: Addr=%h Data=%h Prot=%b", tr.addr, tr.data, tr.arprot),
              UVM_MEDIUM)
  endtask
endclass
