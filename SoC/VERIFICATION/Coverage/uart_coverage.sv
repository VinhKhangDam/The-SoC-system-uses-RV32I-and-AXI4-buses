class uart_coverage extends uvm_subscriber #(uart_transaction);
   ...
   function void write(uart_transaction t);
      ...
      sample_count++;
      if (t.op == UART_AXI_WRITE)
        write_count++;
      else if (t.op == UART_AXI_READ)
        read_count++;
      else
        rx_byte_count++;
   ...
   function void report_phase(uvm_phase phase);
      ...
      `uvm_info("UART_COV", $sformatf("Counts         : WR=%0d RD=%0d RX_BYTE=%0d",
         write_count, read_count, rx_byte_count), UVM_LOW)
      `uvm_info("UART_COV", $sformatf("Op Coverage    : %.2f%%", uart_cg.cp_op.get_coverage()), UVM_LOW)
      `uvm_info("UART_COV", $sformatf("Addr Coverage  : %.2f%%", uart_cg.cp_addr.get_coverage()), UVM_LOW)
      `uvm_info("UART_COV", $sformatf("PROT Coverage  : %.2f%%", uart_cg.cp_prot.get_coverage()), UVM_LOW)
      `uvm_info("UART_COV", $sformatf("TX Byte Cover  : %.2f%%", uart_cg.cp_tx_byte.get_coverage()), UVM_LOW)
      `uvm_info("UART_COV", $sformatf("Baud Coverage  : %.2f%%", uart_cg.cp_baud.get_coverage()), UVM_LOW)
      `uvm_info("UART_COV", $sformatf("TOTAL Coverage : %.2f%%", uart_cg.get_inst_coverage()), UVM_LOW)
      `uvm_info("UART_COV", "======================================================", UVM_LOW)
   endfunction
endclass
