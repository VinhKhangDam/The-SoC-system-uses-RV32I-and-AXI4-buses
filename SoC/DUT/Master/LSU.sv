module LSU (
  ...
  always_comb begin
    case (funct3[1:0])
      ...
      endcase
      2'b01: wdata_comb = (addr_i[1]) ? {data_i[15:0], 16'b0} : {16'b0, data_i[15:0]};
      default: wdata_comb = data_i;
    endcase
  end
  ...
  always_ff @(posedge clk or negedge rstn) begin
    if (!rstn) begin
      ...
      if (data_done) wait_req_drop <= 1'b1;
      else if (!req_i) wait_req_drop <= 1'b0;
  ...
  always_comb begin
    ...
    unique case (PresentState)
      ...
      ST_IF_ADDR: begin
        m_axi_arvalid = 1'b1;
        if (m_axi_arready) NextState = ST_IF_DATA;
      end
      ...
      ST_R_ADDR: begin
        ...
        //if_stall_o    = 1'b1;
        m_axi_arvalid = 1'b1;
        if (m_axi_arready) NextState = ST_R_DATA;
      end
